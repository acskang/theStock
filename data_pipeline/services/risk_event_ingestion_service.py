...
def ingest_risk_events(
    *,
    ...
    log_entry = start_ingestion_log(
        ...
        target_type=DataIngestionLog.TARGET_RISK,
    ...
    try:
        for stock in target_stocks:
            try:
                rows = provider.get_risk_event_rows(stock, days)
                if not rows:
                    ...
                    if not dry_run:
                        record_collection_status(
                            ...
                            row_count=0,
                            message="No risk event rows returned.",
                        )
                    continue
                for row in rows:
                    if not dry_run:
                        lookup = {"stock": stock, "source_key": row.source_key} if row.source_key else {
                            "stock": stock,
                ...
                logger.exception("Risk event ingestion failed for %s", stock.code)
        ...
        if report.failed_count and report.success_count:
            ...
            report.error_message = "Risk event ingestion failed."
    except Exception as exc:
        report.status = DataIngestionLog.STATUS_FAILED
        report.error_message = str(exc)
        logger.exception("Risk event ingestion crashed")
    ...
    update_provider_status(
        provider=provider.provider_name,
        data_type=DataProviderStatus.TYPE_RISK,
        status=report.status,
